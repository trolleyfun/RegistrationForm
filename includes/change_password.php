...
<section id="edit-password">
    ...
    <form action="" method="post" class="form-container">
        ...
        <?php displayErrorMessage($err_change_password['current_password_empty'], "Это поле не может быть пустым"); ?>
        <?php displayErrorMessage($err_change_password['current_password_empty'], "Неверный пароль"); ?>    
        <input type="password" name="new_password_1" placeholder="Новый пароль" class="form-input" required>
        <?php displayErrorMessage($err_change_password['new_password_empty'], "Это поле не может быть пустым"); ?>
        <?php displayErrorMessage($err_change_password['new_password_equal'], "Пароли должны совпадать"); ?>
